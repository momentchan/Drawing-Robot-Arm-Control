...
extern bool write;
extern vector<Eigen::Vector3f> trace;
extern vector<vector<Eigen::Vector2f>> sketchPositionXY;
extern float z_axis_black;
...
inline void PositionRampGenerator()
{
	if(!MOVL_newflag){
		if( MOVL_L == true)
		{
			...
			if(write)
				outputFile<<P07Cmd(0)<<" "<<P07Cmd(1)<<" "<<P07Cmd(2)<<endl;
		}
	}else{
		if(traceIndex!=sketchPositionXY[sketchIndex].size()-1){
			Vector3f point;
			point(0) = sketchPositionXY[sketchIndex][traceIndex](0);
			point(1) = sketchPositionXY[sketchIndex][traceIndex](1);
			point(2) = z_axis_black;
			P07Cmd = point;
			cout<<P07Cmd(0)<<" "<<P07Cmd(1)<<" "<<P07Cmd(2)<<endl;
			traceIndex++;
		}else{
			MOVLDEC_L = false;
			MOVL_L = false;
		...
		/*if(abs(1000.0f*(T07Cmd(0,3)-T07(0,3)))<1.0&&abs(1000.0f*(T07Cmd(1,3)-T07(1,3)))<1.0){
			MOVLDEC_L = false;
			MOVL_L = false;
		}
		P07Cmd = P07Trgt;*/
...
void ServoLoop()
{
	...
	else if( modeMotion_C == true)
	{
		...
		{	
			...
			//ENC7_e = 1.1f*(ENC7 + ENC7_BL) - 0.2f*ENC7_1 + 0.1f*ENC7_2;
			ENC7_e_2 = ENC7_e_1; ENC7_e_1 = ENC7_e;
			
			deltaZ_FS = 0.019f - 0.06f*tanf( 0.3066763 - rev2rad*ENC7_e/267168.0f );
			//deltaZ_FS = 0.019f - 0.06f*tanf( 0.3066763 - rev2rad*((float)ENC7)/267168.0f );
			...
			{
				d3_d = d3_0 - 0.019f + 0.06f*tanf( 0.3066763f - rev2rad*((float)ENC7)/267168.0f );
				//d3_d = d3_0 - 0.019f + 0.06f*tanf( 0.3066763f - rev2rad*((float)(ENC7+ENC7_BL))/267168.0f );
				//d3_d = d3_0 - deltaZ_FS;
			}
			...
			if( FORCE_CONTROL == true)
			{
				...
				if( FORCE_TRACKING == true)// abs(Fz_th) > abs(Fz_task)
				{
					...
					else if( abs(error_Fz) > 0.4f && abs(error_Fz) <= 0.8f)
					{
						...
						z07Cmd_temp = z07Cmd_temp_1 + ( 0.02f*error_Fz + 0.005f*summ_error_Fz )/Kz_FS;
						//P07Cmd(2,0) = z07Cmd_temp_1 + ( 0.002f*error_Fz + 0.005f*summ_error_Fz )/Kz_FS;
						//P07Cmd(2,0) = P07Cmd_1(2,0) + ( 0.002f*error_Fz + 0.005f*summ_error_Fz )/Kz_FS;
					}
					else
					{
						summ_error_Fz = error_Fz_1 + error_Fz;
						z07Cmd_temp = z07Cmd_temp_1 + ( 0.025f*error_Fz + 0.01f*summ_error_Fz )/Kz_FS;
						//P07Cmd(2,0) = P07Cmd_1(2,0) + ( 0.0025f*error_Fz + 0.01f*summ_error_Fz )/Kz_FS;
